...
function isMechanicalMigrationRerun(err) {
  return !!(err && (err.code === 'RERUN_MECHANICAL' || err.name === 'MechanicalMigrationRerunError'));
}
...
function printShortManualGuide(discoveryLine, manualGuideLines = []) {
  ...
  console.log(chalk.gray('  3) 아래에서 y를 입력하면 현재 작업을 재실행합니다.'));
...
async function stopAndOfferGeminiApply(opts) {
  ...
  const key = `${String(discoveryLine)}|${keySources
    .map((s) => String(s))
    .sort()
    .join(',')}`;
